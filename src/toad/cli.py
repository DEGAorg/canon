...
from toad.strategy_selection import strategies
...
def acp(
    ...
    agent_data: AgentData = {
        ...
        "short_name": "agent",
        "url": "https://github.com/DEGAorg/canon",
        "protocol": "acp",
        "type": "coding",
    ...
    print("https://github.com/DEGAorg/canon/discussions")
...
_CANON_GIT_URL = "git+https://github.com/DEGAorg/canon.git"
...
@click.option(
    ...
    default="main",
    show_default=True,
    help="Branch (or tag) to install from the canon-app repo.",
)
@click.option(
    "--check",
    is_flag=True,
...
def update(branch: str, check: bool) -> None:
    ...
    if check:
        click.echo(f"Local:  canon-app {local_version}")
        remote = _fetch_remote_version(branch)
        click.echo(
            f"Remote: canon-app {remote} (branch={branch})"
            if remote
    ...
    spec = f"canon-app @ {_CANON_GIT_URL}@{branch}"
...
def _fetch_remote_version(branch: str) -> str | None:
    ...
    url = (
        "https://raw.githubusercontent.com/DEGAorg/canon/"
...
def _read_installed_version() -> str | None:
    """Read the version of the canon-app install that's currently on PATH."""
    import subprocess
