...
Socket path: ~/.canon/sockets/toad-{pid}.sock (auto-cleaned on shutdown).
...
SOCKET_DIR = Path.home() / ".canon" / "sockets"
...
async def _dispatch(app: App, request: dict[str, Any]) -> dict[str, Any]:
    ...
    if cmd == "room":
        ...
        try:
            service = await app.screen.query_one(ChatView).room_service()
            return await execute(service, request)
        except (RoomError, OSError, sqlite3.Error, NoMatches) as exc:
    ...
    if cmd == "snapshot":
        widgets = [_serialize_widget(w) for w in app.screen.query("*")]
    ...
    if cmd == "query":
        ...
        widgets = [_serialize_widget(w) for w in app.screen.query(selector)]
        return {"widgets": widgets}
    ...
    if cmd == "action":
        ...
        if bare_name in ("open_subagent_tab", "close_subagent_tab"):
            ...
            if bare_name == "open_subagent_tab":
                ...
                if err is not None:
                    return {"error": err}
                await cast("MainScreen", app.screen).action_open_subagent_tab(
                    name=tab_name, objective=objective
                )
            else:
                await cast("MainScreen", app.screen).action_close_subagent_tab(name=tab_name)
            return {"ok": True}
        if bare_name == "set_pane_width":
            ...
            try:
                cast("MainScreen", app.screen).action_set_pane_width(width)
            except ValueError as exc:
                return {"error": str(exc)}
            return {"ok": True}
        if bare_name == "get_pane_width":
            return {"ok": True, "width": cast("MainScreen", app.screen).action_get_pane_width()}
        await app.run_action(name)
        return {"ok": True}

    if cmd == "subagent_status":
        status = cast("MainScreen", app.screen).subagent_status()
        return {
            "ok": True,
...
async def start_socket_server(app: App) -> asyncio.AbstractServer:
    ...
    if not stat.S_ISDIR(info.st_mode) or info.st_uid != os.getuid():
        raise PermissionError(f"Unsafe controller socket directory: {SOCKET_DIR}")
    SOCKET_DIR.chmod(0o700)
    path = _socket_path()
    if path.is_symlink() or (path.exists() and not stat.S_ISSOCK(path.lstat().st_mode)):
        raise PermissionError(f"Refusing to replace non-socket: {path}")
