...
from toad.extensions.dega_panel.install_store import is_installed, strategies_dir
...
class BuildRow(ListItem):
    ...
    def compose(self) -> ComposeResult:
        ...
        score = f"  [dim]{b.score:.2f}[/]" if b.score is not None else ""
        origin = "  [dim]Curated Core template[/]" if b.origin == "core" else ""
        head = f"[b]{b.name}[/]{crown}{score}{origin}  {inst}"
        summary = b.summary if len(b.summary) <= 72 else b.summary[:69].rstrip() + "…"
        body = f"{head}\n[dim]{summary}[/]"
...
class UtilityView(Vertical):
    ...
    UtilityView ListItem.--highlight { background: $accent 20%; text-style: bold; }
    UtilityView Button { height: 3; width: auto; min-width: 16; }
    UtilityView Button:focus { text-style: reverse; }
    UtilityView #setup-next { height: auto; margin: 1 0; color: $text-muted; }
    UtilityView #strategy-hints { height: auto; color: $text-muted; }
    ...
    def compose(self) -> ComposeResult:
        ...
        with VerticalScroll(id="build-detail"):
            ...
            yield Static(id="setup-next", markup=False)
            yield Static("↑/↓ choose · a access · i install/remove · r dry-run · x stop",
                         id="strategy-hints", markup=False)
    ...
    def refresh_detail(self) -> None:
        ...
        submission = f"  [dim]#{b.submission_id}[/]" if b.submission_id is not None else ""
        origin = "  [dim]Curated Core template[/]" if b.origin == "core" else ""
        self.query_one("#detail-head", Static).update(
            f"[b]{b.name}[/]{submission}{origin}\n"
            f"[dim]{b.summary}[/]"
        )
        ...
        self.query_one("#detail-rule", Static).update(
            f"{state_label}\n{counts}{access_text}\n"
            "Editable template — setup and dry-run validation happen in your project."
        )

        installed = is_installed(b.key)
        if installed:
            next_step = (
                f"Installed template: {strategies_dir() / b.key}\n"
                "Next: review configuration and required credentials before dry-run (r)."
            )
        elif state == "active":
            next_step = "Access active · Not installed\nNext: press i to download the template."
        else:
            next_step = "Not installed\nNext: check access, then install and configure the template."
        self.query_one("#setup-next", Static).update(next_step)
