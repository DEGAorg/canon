...
build's own `Rule` so the UI keeps working out of the box.
"""
...
def rule_for(key: str) -> Rule:
    """Resolve the gating rule for ``key``: config override else build default."""
    cfg = _load()
    override = _rule_from_config(cfg, key)
    if override is not None:
        return override
    build = BUILDS_BY_KEY.get(key)
    return build.rule if build else Rule()
