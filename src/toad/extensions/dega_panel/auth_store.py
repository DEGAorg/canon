...
#
# JSON-RPC endpoint. Default (unset) is the public Sepolia RPC, which is what
# the demo registry below is deployed on. For Ethereum mainnet use a mainnet RPC.
DEGA_CHAT_RPC=https://ethereum-sepolia-rpc.publicnode.com
#
# DegaChatRegistry smart-contract address (Sepolia burnFrom, one-node-per-address +
# owner lookup + reverse pubkey lookup). Point this at your mainnet registry to
# go live.
DEGA_CHAT_REGISTRY=0x2D69074C1ceCe968529CdC08162B31700F7Aed4A
#
# $DEGA token address (MockDEGA on Sepolia). Optional: if unset the registry's
# degaToken() is read on-chain, so you normally leave this blank.
# DEGA_CHAT_TOKEN=0x4D55b1B62EdF27a8154F7bD810a3EcCEd6326896
#
...
def _ensure_dir() -> None:
    ...
    try:
        ...
        pass
    # Seed a dega-chat.env with placeholders on first run so a fresh install
    # knows what to configure instead of silently falling back to Sepolia.
