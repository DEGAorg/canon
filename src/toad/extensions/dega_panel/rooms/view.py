...
class RoomsView(Vertical):
    ...
    RoomsView .room-controls Button { min-width: 8; width: auto; margin-left: 1; }
    RoomsView #room-settings { max-height: 45%; overflow-y: auto; }
    RoomsView #room-send-hint { height: 1; color: $text-muted; }
    ...
    RoomsView #room-owner-controls { height: auto; }
    RoomsView #room-owner-controls Button { width: 1fr; margin: 0 0 1 0; }
    RoomsView #room-owner-controls Select { margin-bottom: 1; }
    RoomsView #room-owner-controls Input { margin-top: 1; }
    RoomsView .settings-label { height: auto; color: $text-muted; }
    ...
    def __init__(self, chat: ChatView) -> None:
        ...
        self.creating = False
        self._create_return_to_room = False
        self.pending_send: tuple[str, str, set[str]] | None = None
        self.history_ids: dict[str, set[str]] = {}
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="room-main"):
            ...
            with Vertical(classes="room-controls", id="room-create-form"):
                ...
                with Horizontal(classes="room-controls"):
                    ...
                    yield Button("Cancel", id="room-create-cancel")
            ...
            with Collapsible(title="Members and settings", id="room-settings", collapsed=True):
                yield Label("Members", classes="settings-label")
                ...
                with Vertical(id="room-owner-controls"):
                    ...
                    yield Button("Invite contact", id="room-invite", variant="primary")
                    yield Button("Remove / revoke", id="room-remove", variant="error")
                    yield Input(placeholder="New room name", id="room-rename-input")
                    yield Button("Rename", id="room-rename", variant="primary")
                    yield Button("Close room", id="room-close", variant="error")
                yield Button("Leave room", id="room-leave", variant="error")
            ...
            yield Static("Enter send · Tab next", id="room-send-hint", markup=False)
    ...
    async def refresh_rooms(self) -> None:
        ...
        async with self.render_lock:
            ...
            self.chat.query_one("#rooms-section", Collapsible).title = f"Rooms · {len(rooms)}"
            ...
            if self.selected:
                self.history_ids[self.selected] = {message["id"] for message in history}
            ...
            self.query_one("#room-retry").display = unresolved > 0 and not self.creating
            ...
            sending = bool(pending and pending[0] == self.selected)
            for message in history:
                if (sending and pending and message["id"] not in pending[2]
                        and message["author"] == self.service.me
                        and message["text"] == pending[1]):
                    continue
                await self._mount_message(scroll, message)
            if sending and pending:
                await scroll.mount(Horizontal(
                    Static(Text(f"You · Sending…\n{pending[1]}"), classes="room-line mine"),
                    classes="room-message mine",
                ))
            if history or sending:
    ...
    def _render_controls(self, room: dict | None) -> None:
        ...
        self.query_one("#room-send-hint").display = active
        self._set_send_busy()
        self.query_one("#room-create-form").display = self.creating
        self.query_one("#room-history").display = not self.creating
        if self.creating:
            self.query_one("#room-heading", Label).update("New room")
            self.query_one("#room-state", Static).update("Choose a name for your room.")
            for selector in ("#room-response", "#room-settings", "#room-composer-row",
                             "#room-send-hint"):
                self.query_one(selector).display = False
        if room:
            names = [
    ...
    async def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if action == "new":
            if not (self.display and self.creating):
                self._create_return_to_room = self.display
            self.creating = True
            self._render_controls(None)
            self.chat.show_room(True)
            await self.activate()
            await self.refresh_rooms()
        ...
        if action == "create-cancel":
            self.creating = False
            await self.refresh_rooms()
            self.chat.show_room(self._create_return_to_room)
            self.chat.query_one("#room-new", Button).focus()
            return
        if action == "send":
            self._start_send()
            return
    ...
    def _set_send_busy(self) -> None:
        busy = self.pending_send is not None
        self.query_one("#room-composer", Input).disabled = busy
        self.query_one("#room-send", Button).disabled = busy

    def _start_send(self) -> None:
        if self.pending_send is not None or not self.selected:
            return
        composer = self.query_one("#room-composer", Input)
        body = composer.value
        if not body.strip():
            return
        room_id = self.selected
        self.pending_send = (room_id, body, self.history_ids.get(room_id, set()).copy())
        self.drafts[room_id] = ""
        composer.value = ""
        self._set_send_busy()
        self.run_worker(self._send_message(room_id, body), group="room-send")
    ...
    async def _send_message(self, room_id: str, body: str) -> None:
        try:
            ...
            async with self.action_lock:
                await self.service.send(room_id, body)
            self.query_one("#room-feedback", Static).update("")
        except (Exception, asyncio.CancelledError) as exc:
            feedback = f"Could not send: {exc}. Retry available."
            saved = False
            if self.service is not None and self.pending_send is not None:
                try:
                    history = await self.service.history(room_id)
                    saved = any(
                        message["id"] not in self.pending_send[2]
                        and message["author"] == self.service.me
                        and message["text"] == body
                        for message in history
                    )
                except (RoomError, OSError, sqlite3.Error) as history_error:
                    feedback += f" Could not check saved delivery: {history_error}."
            if not saved:
                self.drafts[room_id] = body
                if self.selected == room_id:
                    self.query_one("#room-composer", Input).value = body
            self.query_one("#room-feedback", Static).update(feedback)
            ...
            await self.refresh_rooms()
            if self.selected == room_id:
                self.query_one("#room-composer", Input).focus()
