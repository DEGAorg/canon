...
class AuthBar(Horizontal):
    ...
    connected, a red "Sign out" chip is shown next to the email (clickable /
    'o') so the user can drop the session (clears ~/.canon/auth.json).
    """

    DEFAULT_CSS = """
    AuthBar { height: auto; padding: 0 1 1 1; border-bottom: solid $panel-lighten-2; }
    ...
    AuthBar #btn-logout {
      ...
      background: $error 30%; color: $text; text-style: bold;
      content-align: center middle;
    }
    AuthBar #btn-logout:hover { background: $error; color: #ffffff; }
    """
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Static("Logout", id="btn-logout")
    ...
    def set_status(self, status: FlowStatus) -> None:
        ...
        b = self.query_one("#btn-logout", Static)
    ...
    def on_click(self, event) -> None:
        """Any click in the bar when connected logs out — robust against the
        Button label not painting on some skins."""
        if self._status.phase == "connected":
            # Imported here to avoid an import cycle (auth_view <- pane).
            # Walk UP to the DegaPane ancestor (self lives inside it); the pane
            # is not queryable from self.app when mounted inside a Screen.
...
async def _apply_approved(client: DeviceClient, approved: dict) -> AuthState:
    ...
    if email:
        ...
        except Exception:  # noqa: BLE001 - elements are best-effort
            elements_data = {}
            total = 0
            addresses = []
