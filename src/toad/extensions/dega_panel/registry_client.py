...
class ChainRegistry:
    ...
    Talks to a deployed registry on an EVM chain (currently the Sepolia test
    deployment). RPC URL, contract address and signer come from env so keys are
    never hardcoded:

    - ``DEGA_CHAT_RPC`` — JSON-RPC endpoint (defaults to a public Sepolia RPC).
    - ``DEGA_CHAT_REGISTRY`` — the DegaChatRegistry address.
    - ``DEGA_CHAT_PK`` — the signer private key (testnet). Falls back to reading
    ...
    def __init__(
        ...
        *,
        ...
        self._rpc = rpc or _read_chat_env_key("DEGA_CHAT_RPC") or os.environ.get("DEGA_CHAT_RPC") or _DEFAULT_SEPOLIA_RPC
        self._registry = registry or _read_chat_env_key("DEGA_CHAT_REGISTRY") or os.environ.get("DEGA_CHAT_REGISTRY") or _DEFAULT_SEPOLIA_REGISTRY
    ...
    def open_node(
        ...
        *, nostr_secret: str | None = None,
        ...
        # If the fee is >0, the contract calls degaToken.transferFrom(msg.sender, this, fee).
    ...
    def _ensure_fee_approved(self, fee: int | None = None) -> None:
        ...
        self._send(tx, {"action": "approve_fee", "username": "", "fee": fee})
    ...
    def _revert_reason(self, tx: TxParams, block_number: int | None) -> str | None:
        ...
        try:
            self._w3.eth.call(tx, block_identifier=block)  # type: ignore[arg-type]
    ...
    def _friendly_revert_message(self, reason: str) -> str:
        ...
        if "insufficient balance" in low:
            return "your wallet does not have enough MockDEGA to pay the fee"
...
_DEFAULT_SEPOLIA_REGISTRY = "0x2D69074C1ceCe968529CdC08162B31700F7Aed4A"  # Sepolia: ownership proof, burnFrom fee, registration TTL
_DEFAULT_SEPOLIA_TOKEN = "0x4D55b1B62EdF27a8154F7bD810a3EcCEd6326896"  # fallback only; real token is read from the registry
...
class RegistryClient:
    ...
    - ``chain``: real on-chain via ChainRegistry (web3 → Sepolia test deploy).
