...
class ElementsBar(Static):
    ...
    def render(self) -> str:
        ...
        if logged_in and auth is not None and auth.session:
            ...
            return f"[b]Elements[/] [dim]· {total} total[/]\n{chips}"
...
class DegaPane(Vertical):
    ...
    auth: reactive[AuthState] = reactive(AuthState, init=False)
    ...
    def on_tabbed_content_tab_activated(self, event: TabbedContent.TabActivated) -> None:
        if event.tabbed_content.id != "dega-subtabs":
            return
        self.query_one(ElementsBar).display = event.pane.id == "tab-dega-utility"
