...
class ChatView(Vertical):
    ...
    ChatView #side {
        width: 24; background: $panel 25%; border-right: solid $panel-lighten-2;
        height: 1fr;
    }
    ...
    ChatView #room-new { height: 1; border: none; min-width: 10; }
    ...
    ChatView.compact #side {
        width: 1fr; height: auto; max-height: 45%;
    ...
    ChatView ListItem.--highlight { background: $boost; }
    ...
    ChatView #add-contact-section { height: auto; padding: 0; }
    ChatView #btn-add-contact { width: 1fr; min-width: 10; }
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal(id="chat-wrap"):
            with VerticalScroll(id="side"):
                ...
                yield Button("Renew", id="btn-renew-registration", compact=True)
                yield Button("Retry", id="btn-retry-registration", compact=True)
                with Collapsible(title="Contacts", id="contacts-section", collapsed=False):
                    yield ListView(id="contacts")
                    with Collapsible(title="Add contact", id="add-contact-section", collapsed=True):
                        yield Input(placeholder="Username", id="contact-name")
                        yield Button("Add contact", id="btn-add-contact", variant="primary",
                                     compact=True)
                with Collapsible(title="Rooms", id="rooms-section", collapsed=False):
                    yield ListView(id="rooms-list")
                    yield Button("New room", id="room-new")
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        if event.button.id == "room-new":
            ...
            self.run_worker(self._send_and_rebuild(), exclusive=True, name="send")
    ...
    async def _send_message(self) -> None:
        ...
        self._sent.append({
            ...
            "timestamp": timestamp, "to": recipient, "event_id": event_id,
        })
        self._drafts.pop(recipient, None)
        if (self._selected_contact or {}).get("pubkey") != recipient:
            await self._render_conversation()
            return
        if composer.value.strip() == text:
            composer.value = ""
        self.notify("message sent (E2E)")
        await self._render_conversation()
    ...
    async def _render_actions(self) -> None:
        ...
        recipient = (self._selected_contact or {}).get("pubkey")
        composer.disabled = not bool(recipient)
        send_btn.disabled = not bool(recipient)
        clear_btn.disabled = not bool(recipient)
        ...
        self.query_one("#add-contact-section").display = registered
