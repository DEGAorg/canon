...
class DagNode(Widget, can_focus=True):
    ...
    DagNode.status-stopped { border: round $surface-lighten-2; color: $text-muted; }
    DagNode.status-error { border: round $error; color: $error; }
    ...
    def set_status(self, status: str) -> None:
        ...
        self._status = status
        self.remove_class(
            "status-done", "status-running", "status-pending", "status-stopped", "status-error"
        )
