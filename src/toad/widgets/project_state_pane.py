...
from toad.dega.registry import discover as discover_dega
...
PANEL_ROUTES: dict[str, tuple[str, str]] = {
    ...
    "dega": (SECTION_DEGA, "tab-dega"),
    "utility": (SECTION_DEGA, "tab-dega"),
    "elements": (SECTION_DEGA, "tab-dega"),
    "chat": (SECTION_DEGA, "tab-dega"),
...
class ProjectStatePane(Vertical):
    ...
    ProjectStatePane #section-dega {
        border-left: tall gold 30%;
    }
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._dega_timer: Timer | None = None
        self._dega_mounted: bool = False
        ...
        self._dega_panel: DegaPanel | None = discover_dega()
        ...
        if self._dega_panel is not None:
            self._sections.append(_SectionDef(SECTION_DEGA, self._dega_panel.title))
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(id=SECTION_CONTEXT, classes="pane-section"):
            ...
            with TabbedContent(id=TABS_CONTEXT):
                # Keep the ACP handler's widget mounted; hide its tab on mount.
        ...
        # --- DEGA section (conditional) ---
        # Same host-owns-the-slot contract as Growth: the panel plugin fills
        # ``#dega-container`` on first show via ``DegaPanel.mount(container)``.
        if self._dega_panel is not None:
            dega = self._dega_panel
            with Vertical(id=SECTION_DEGA, classes="pane-section"):
                yield SectionStatusBadge(
                    BadgeState.POLLING, id=BADGE_DEGA, classes="section-badge"
                )
                with TabbedContent(id=TABS_DEGA):
                    with TabPane(dega.title, id="tab-dega"):
                        yield Vertical(id="dega-container")

    def on_mount(self) -> None:
        # All sections start hidden; the user opens one via toolbar / chat.
        for section in self.query(".pane-section"):
            section.display = False
        context_tabs = self.query_one(f"#{TABS_CONTEXT}", TabbedContent)
        context_tabs.hide_tab("tab-plan")
        context_tabs.active = "tab-files"
        self._sync_toolbar()
    ...
    def _sync_dega_timer(self, section_id: str, *, visible: bool) -> None:
        """Mount the DEGA panel on first show; poll only if it asked for it."""
        if section_id != SECTION_DEGA or self._dega_panel is None:
            return
        if visible:
            self._fetch_dega()
            interval = self._dega_panel.refresh_seconds
            if self._dega_timer is None and interval:
                self._dega_timer = self.set_interval(interval, self._fetch_dega)
            # Deck el foco a un widget interno del panel DEGA para que los
            # bindings del panel (l login, 1/2/3 elements, t tabs) disparen —
            # Textual solo activa los bindings de un widget si él o un hijo
            # tiene foco.
            self.call_after_refresh(self._focus_dega_panel)
        else:
            self._stop_dega_timer()

    def _focus_dega_panel(self) -> None:
        try:
            from toad.extensions.dega_panel.pane import DegaPane
            pane = self.query_one(DegaPane)
            # Utility por defecto -> build-list
            target = pane.query_one("#build-list")
            target.focus()
        except Exception:  # noqa: BLE001 - focus is best-effort
            pass

    def _stop_dega_timer(self) -> None:
        if self._dega_timer is not None:
            self._dega_timer.stop()
            self._dega_timer = None
    ...
    def show_section(self, section_id: str) -> None:
        ...
        self._sync_dega_timer(section_id, visible=True)
    ...
    def show_single_section(self, section_id: str) -> None:
        ...
        for sec in self._sections:
            ...
            self._sync_dega_timer(sec.section_id, visible=visible)
        self._sync_toolbar()
    ...
    def _register_plan_exec_section(self) -> None:
        """No-op: the "Plans" nav button/section is hidden per product decision.

        The ACP handler stays wired (a plan can still be opened by panel route),
        but nothing is surfaced in the UI toolbar. Kept as a method so
        ``configure_plan_execution``'s contract holds.
        """
        return
    ...
    async def _fetch_dega(self) -> None:
        """Drive the DegaPanel lifecycle.

        First call mounts the panel into ``#dega-container``; later calls
        invoke ``panel.refresh()``. The host owns the badge bookkeeping so
        the panel stays oblivious to UPDATING / ERROR / POLLING.
        """
        panel = self._dega_panel
        if panel is None:
            return
        try:
            container = self.query_one("#dega-container", Vertical)
        except NoMatches:
            return

        try:
            if not await panel.available():
                self._mark_badge(BADGE_DEGA, state=BadgeState.OFFLINE)
                return
            # Keep the DEGA badge in POLLING while the panel mounts so the UI
            # does not look stuck in UPDATING during the initial compose.
            self._mark_badge(BADGE_DEGA, state=BadgeState.POLLING)
            if not self._dega_mounted:
                await panel.mount(container)
                self._dega_mounted = True
            await panel.refresh()
        except Exception as exc:
            log.warning("DEGA fetch failed: %s", exc)
            self._mark_badge(
                BADGE_DEGA, state=BadgeState.ERROR, message=str(exc)[:30]
            )
            return
        self._mark_badge(BADGE_DEGA, state=BadgeState.POLLING, updated=True)
