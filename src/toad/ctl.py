...
def find_socket() -> str:
    """Find a live Canon/Toad socket in ~/.canon/sockets/.

    Tries each matching socket, removes stale ones (no listener),
    and returns the first that accepts a connection.
    """
    socks = sorted(glob.glob(str(Path.home() / ".canon/sockets/toad-*.sock")))
    if not socks:
        print("error: no canon socket found in ~/.canon/sockets/", file=sys.stderr)
        sys.exit(1)
    ...
    print("error: no live canon socket found in ~/.canon/sockets/", file=sys.stderr)
    sys.exit(1)
