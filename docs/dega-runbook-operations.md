...
The historical contract notes below apply to **testnet (Sepolia)**. Current defaults use
Ethereum mainnet; see [configuration.md](dega/configuration.md). Real live trading with funds is blocked and requires a Polygon
...
multi-node and reverts `usernameOfOwner`** — it is not usable. The former shipped default was the 09-15 v3
registry `0xf3D30Cf5…`, and the historical TTL deployment was `0x50600d8D…`; both use MockDEGA
