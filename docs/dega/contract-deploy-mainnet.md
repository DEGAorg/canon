...
The production registry is deployed at `0x4c698AC2f25dD82386658080223583e0EEbB523f`.
Its verified settings and read-only probes are in [contracts.md](contracts.md).
New installations default to this registry. Existing config files and registrations are
preserved; registrations do not migrate between registries automatically.
...
4. For a future deployment, update the generated configuration template and registry client
   defaults together, with tests. Existing client config files require an explicit user edit;
   package upgrades must not overwrite them.
