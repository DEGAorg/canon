...
### Production default (verified 2026-09-18)
...
| DEGA token | `0x97aeE01ed2aabAd9F54692f94461AE761D225f17` (18 decimals) |
| Fee | 6,719,270 DEGA (`6719270000000000000000000` base units) |
| Registration duration | 365 days (`31536000` seconds) |
| Users per node | 10 |
| Contract owner | `0x6E6B4CC3B93D7405b590FDc120CA8Df36639DdDB` |

These values were read from the deployed contract. Fee, duration and capacity can change through
owner transactions; the client reads them on-chain. New config files select this registry;
upgrading never overwrites existing `~/.canon/dega-chat.env` files or migrates registrations.

### Historical Sepolia deployments

The following records describe testnet state verified on 2026-09-16, not current defaults.
The later burnable MockDEGA deployment is recorded in
[the 2026-09-17 testnet report](sepolia-burnfrom-2026-09-17.md).
...
| **09-15 v3 (FORMER DEFAULT)** | **`0xf3D30Cf5FEBCa40Ff1E6A7254BEE34f170fAb088`** | **`0x5f431e1a97b18C2a81c625E01B643BC836592455`** | 5e7 = 0.00000000005 $DEGA (18 decimals) | one-node-per-address + `usernameOfOwner` + `usernameForPubkey`; **no `registrationTTL`** |
| **09-16 TTL (HISTORICAL TESTNET)** | **`0x50600d8D8BA6F36F51A81d70B891e7cC81E8C8fe`** | same MockDEGA `0x5f431e1a97b18C2a81c625E01B643BC836592455` | 5e7 | adds `registrationTTL` (**86400 s**) + `renewNode`/`registrationOfOwner`; per-member expiry (`membershipPeriod`/`memberExpiry`) is **absent** — this is the shipped TTL design, not the reverted per-member experiment |

> **Reusing a token preserves its balances, not registry state.** Pointing a new registry at the EXISTING MockDEGA
...
> (`DEGA_CHAT_REGISTRY` / `DEGA_CHAT_RPC`) — see [configuration.md](configuration.md).

> Historical testnet fee format: fee() = 50000000 base units, token decimals 18 → **0.00000000005 $DEGA** (an
