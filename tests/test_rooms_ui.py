...
async def test_create_room_and_cancel_close(tmp_path, monkeypatch, width):
    ...
    async with app.run_test(size=(width, 42)) as pilot:
        ...
        assert view.query_one("#room-send-hint").display
        assert app.query_one("#rooms-section", Collapsible).title == "Rooms · 1"
        ...
        await settle_room_ui(pilot)
        view.query_one("#room-close", Button).focus()
        await settle_room_ui(pilot)
        await pilot.wait_for_scheduled_animations()
        assert await pilot.click("#room-close")
        await settle_room_ui(pilot)
...
async def test_add_contact_is_available_while_viewing_room(tmp_path, monkeypatch, width):
    ...
    async with app.run_test(size=(width, 42)) as pilot:
        ...
        await settle_room_ui(pilot)
        assert await pilot.click("#contact-new")
        await settle_room_ui(pilot)
        assert await pilot.click("#contact-new")
        await settle_room_ui(pilot)
        ...
        assert field.region.width > 20
        assert chat.query_one("#btn-add-contact").is_on_screen
        assert await pilot.click("#contact-cancel")
        await settle_room_ui(pilot)
...
@pytest.mark.parametrize("failure", [None, "room", "unexpected", "cancelled", "saved-cancelled", "saved-unexpected"])
async def test_pending_room_send_is_immediate_and_refresh_safe(tmp_path, monkeypatch, failure):
    """Slow sends clear and lock the composer without duplicating journal messages."""
    ...
    async with app.run_test(size=(106, 42)) as pilot:
        ...
        await pilot.click("#room-new")
        await settle_room_ui(pilot)
        view.query_one("#room-name", Input).value = "Pending team"
        await pilot.click("#room-create")
        await settle_room_ui(pilot)
        original = view.service.send
        entered, release = asyncio.Event(), asyncio.Event()
        calls = []

        async def slow_send(room_id, body):
            calls.append((room_id, body))
            result = (
                await original(room_id, body)
                if failure is None or failure.startswith("saved-") else None
            )
            entered.set()
            await release.wait()
            if failure in {"unexpected", "saved-unexpected"}:
                raise RuntimeError("Unexpected service failure")
            if failure in {"cancelled", "saved-cancelled"}:
                raise asyncio.CancelledError
            if failure == "room":
                raise RoomError("storage_unavailable", "Storage unavailable")
            return result

        monkeypatch.setattr(view.service, "send", slow_send)
        composer = view.query_one("#room-composer", Input)
        composer.value = "Hello pending"
        composer.focus()
        await pilot.press("enter")
        await asyncio.wait_for(entered.wait(), 5)
        assert composer.value == ""
        assert composer.disabled
        assert view.query_one("#room-send", Button).disabled
        view.send_on_enter()
        await view.refresh_rooms()
        await pilot.pause()
        assert len(view.query(".room-line")) == 1
        assert "Sending…" in str(view.query_one(".room-line", Static).render())
        release.set()
        await settle_room_ui(pilot)
        assert len(calls) == 1
        assert not composer.disabled
        assert not view.query_one("#room-send", Button).disabled
        if failure:
            saved = failure.startswith("saved-")
            assert composer.value == ("" if saved else "Hello pending")
            assert "Could not send" in str(view.query_one("#room-feedback", Static).render())
            assert len(view.query(".room-line")) == (1 if saved else 0)
        else:
            assert composer.value == ""
            assert len(view.query(".room-line")) == 1
            assert "Sending…" not in str(view.query_one(".room-line", Static).render())


@pytest.mark.parametrize("width", [60, 106])
async def test_new_room_replaces_conversation_and_cancel_restores_draft(tmp_path, monkeypatch, width):
    monkeypatch.setattr(auth_store, "CANON_DIR", tmp_path)
    monkeypatch.setattr(chat_presence, "PRESENCE_FILE", tmp_path / "presence.json")
    app = Host(tmp_path)
    async with app.run_test(size=(width, 42)) as pilot:
        ...
        await pilot.click("#room-new")
        await settle_room_ui(pilot)
        view.query_one("#room-name", Input).value = "Existing room"
        await pilot.click("#room-create")
        await settle_room_ui(pilot)
        selected = view.selected
        view.query_one("#room-composer", Input).value = "Keep room draft"
        await pilot.click("#room-new")
        await settle_room_ui(pilot)
        ...
        await settle_room_ui(pilot)
        assert not chat.query_one("#conv-col").display
        for selector in ("#room-history", "#room-settings", "#room-composer-row"):
            assert not view.query_one(selector).display
        assert str(view.query_one("#room-heading", Label).render()) == "New room"
        assert await pilot.click("#room-create-cancel")
        await settle_room_ui(pilot)
        assert view.display and not view.creating
        assert view.selected == selected
        assert view.query_one("#room-history").display
        assert view.query_one("#room-composer", Input).value == "Keep room draft"
        chat.show_room(False)
        chat.query_one("#composer", Input).value = "Keep direct draft"
        await pilot.click("#room-new")
        await settle_room_ui(pilot)
        assert await pilot.click("#room-create-cancel")
        await settle_room_ui(pilot)
        assert chat.query_one("#conv-col").display
        assert chat.query_one("#composer", Input).value == "Keep direct draft"
