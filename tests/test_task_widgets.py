...
async def test_diff_flash_marks_only_changed_cells(
    ...
    async with app.run_test() as pilot:
        ...
        await pilot.pause()
        # Resolve the Status column index from the active column set (the "#"
        # column leads the "all" set, so this is not 0).
        from toad.widgets.task_table import COLUMN_SETS

        status_col = next(
            i
            for i, (header, _fmt) in enumerate(COLUMN_SETS[tbl._column_set])
            if header == "Status"
        )
        assert (sample_tasks[0].id, status_col) in tbl._flash_cells
        assert all(
