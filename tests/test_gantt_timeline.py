...
class TestRenderBarRow:
    ...
    def test_risk_labels_present(self) -> None:
        ...
        assert len(line.plain[LABEL_WIDTH:]) > 0
...
class TestRenderGroupHeader:
    ...
    def test_due_date_marker(self) -> None:
        ...
        line = render_group_header(group, data, track_width=60)
        assert "\u25c6" in line.plain[LABEL_WIDTH:]
...
class TestRenderDateAxis:
    ...
    def test_returns_two_rows(self) -> None:
        ...
        assert len(rows) == 2
        # Each row carries a full-width label zone followed by its track.
        for row in rows:
            assert len(row.plain[:LABEL_WIDTH]) == LABEL_WIDTH
    ...
    def test_gate_markers_in_second_row(self) -> None:
        ...
        rows = render_date_axis(data, track_width=60)
        assert "GA" in rows[1].plain[LABEL_WIDTH:]
...
class TestRenderGantt:
    ...
    def test_full_render_no_crash(self) -> None:
        ...
        assert len(lines) >= 8
        label_text = "\n".join(line.plain[:LABEL_WIDTH] for line in lines)
    ...
    def test_minimal_data(self) -> None:
        ...
        lines = render_gantt(data, track_width=60)
        # At least the two axis rows + separator
        assert len(lines) >= 3
...
class TestRenderTodayRow:
    ...
    def test_returns_label_and_track_when_in_range(self) -> None:
        ...
        label = result.plain[:LABEL_WIDTH]
        track = result.plain[LABEL_WIDTH:]
        assert "TODAY" in label
        assert len(label) == LABEL_WIDTH
        assert TODAY_CHAR in track
...
class TestRenderGanttLabelTrackAlignment:
    ...
    def test_all_labels_have_label_width(self) -> None:
        ...
        for i, line in enumerate(lines):
            assert len(line.plain[:LABEL_WIDTH]) == LABEL_WIDTH, (
                f"Row {i} has no full-width label zone"
            )
    ...
    def test_labels_and_tracks_same_length(self) -> None:
        ...
        track_width = compute_track_width(data.total_days)
        lines = render_gantt(data, track_width)
        # Every row is exactly the label zone plus the track.
        assert lines
        for i, line in enumerate(lines):
            assert len(line.plain) == LABEL_WIDTH + track_width, (
                f"Row {i} width {len(line.plain)} != {LABEL_WIDTH + track_width}"
            )
    ...
    def test_track_width_uses_chars_per_week(self) -> None:
        ...
        lines = render_gantt(data, track_width)
        # Every row fills the label zone plus the whole track.
        for i, line in enumerate(lines):
            assert len(line.plain) == LABEL_WIDTH + track_width, (
                f"Row {i} width {len(line.plain)} != {LABEL_WIDTH + track_width}"
            )
