...
async def test_access_copy_button_and_real_signal_visibility(monkeypatch, tmp_path, width):
    ...
    async with app.run_test(size=(width, 42)) as pilot:
        ...
        detail = str(view.query_one("#detail-rule", Static).render())
        assert "ACCESS" not in detail
        assert "Editable template" in detail
        assert "funded wallet" not in detail
...
@pytest.mark.asyncio
async def test_unavailable_template_does_not_activate(monkeypatch):
    monkeypatch.setattr(utility, "backend_eligibility", AsyncMock(return_value={
        "key": "arbiter", "state": "unavailable", "grant": None, "alternatives": [],
    }))
    activate = AsyncMock()
    monkeypatch.setattr(utility, "activate_access", activate)
    ...
    async with app.run_test(size=(100, 40)) as pilot:
        await pilot.pause()
        view = app.query_one(utility.UtilityView)
        view._selected = "arbiter"
        assert await view._activate("arbiter") is False
        assert "archive unavailable" in str(view.query_one("#op-status", Static).render())
