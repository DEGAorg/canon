...
#
# Finds the socket automatically (first "$HOME"/.canon/sockets/toad-*.sock found).

find_socket() {
  local sock
  sock=$(ls "$HOME"/.canon/sockets/toad-*.sock 2>/dev/null | head -1)
  if [[ -z "$sock" ]]; then
    echo "error: no toad socket found in ~/.canon/sockets/" >&2
    exit 1
  fi
  echo "$sock"
}
